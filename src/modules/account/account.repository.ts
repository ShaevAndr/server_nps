...
export class AccountRepository {
    ...
    async getAccountById(accountId: number): Promise<AccountDocument | null> {
        const account = await this._accountRepository.findOne({ accountId: accountId }).exec();
        return account
    }
    async createAccount(subdomain: string, accountId: number, tokens: SuccessTokenResponse): Promise<Account | null> {
        const newAccount = {
            subdomain,
            accountId,
    ...
    async updateAccountByID(accountId: number, updatedClient: Partial<Account>): Promise<Account | null> {
