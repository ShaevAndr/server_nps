...
import { ConvertedCustomFields } from "./entities/custom-field.entities";
...
export class FieldsServices {
    ...
    public async convertCustomFields(accountId: number, custom_fields: CustomField[]): Promise<ConvertedCustomFields> {
        const loggerContext = `${FieldsServices.name}/${this.convertCustomFields.name}/${accountId}`;
    ...
    public async updateOrCreate(field: CustomFieldDto): Promise<CustomFieldDocument> {
        const loggerContext = `${FieldsServices.name}/${this.updateOrCreate.name}`;
    ...
    public async getField(accountId: number, fieldId: number): Promise<CustomFieldDocument> {
        const loggerContext = `${FieldsServices.name}/${this.getField.name}/${accountId}`;
